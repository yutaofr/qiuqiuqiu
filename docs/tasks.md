...
- [ ] **Task 1: Create `qqq_cycle/config/model_v22.yaml`**
  ...
  **Acceptance criteria:**
  - [ ] File exists at `qqq_cycle/config/model_v22.yaml` with keys for dual-memory windows (104, 260), covariance half-life (78), warmup (260), drift thresholds (1.2, 1.8), IIR delta (0.9), heal threshold (0.25), lambda_rho (0.75), omega_state ([1.0, 0.7, 0.3, 0.6, 0.9]), pct window (520), noise quantile (0.10)
  - [ ] `load_config()` returns a typed object; callers can use dot access

  **Verification:**
  - [ ] `python -c "from qqq_cycle.config import load_config; c = load_config(); assert c.warmup_weeks == 260"`
...
- [ ] Config loads cleanly; no import errors

---

### Phase 2: Micro Layer

- [ ] **Task 2: Core data structures and grace period (`micro_layer.py`)**
  ...
  **Acceptance criteria:**
  - [ ] `MicroDailyState` dataclass tracks per-member age counters, grace period set, and grace expiry
  - [ ] Grace period freezes breadth/correlation counters for missing members ≤ 3 days; sets c_t^{data}=1
  - [ ] `V_tau_20` and `V_tau_60` correctly exclude sub-maturity members and members in grace period
  - [ ] Giant missing weight check (§9.5): if `w_miss > 0.5 * w_(5)` → hold b_τ, c_τ; set c_t^{data}=1

  **Verification:**
  - [ ] Unit test: member added on day 0 is not in V^20 until day 20
  - [ ] Unit test: grace period member excluded from b_τ / c_τ for ≤3 missing days
...
- [ ] **Task 3: Smoothed weights, breadth collapse, correlation concentration**
  ...
  **Acceptance criteria:**
  - [ ] `compute_smoothed_weights()` uses ρ_w = 2^{-1/5}; freezes updates on rule/rebalance days
  - [ ] `compute_breadth()` calls `pit_engine.get_adjusted_window(ticker, τ, 20, asof=τ)` for each member in V^20; if PITAdjustmentEngine unavailable → micro layer halts (raises `MicroLayerUnavailableError`)
  - [ ] `compute_correlation_concentration()` computes normalized weighted average off-diagonal correlation per §9.8 formula
  - [ ] Weekly aggregation: `b_t^{wk}` = median of τ in week(t), same for `c_t^{wk}` (§9.9)

  **Verification:**
  - [ ] Unit test: 100% of stocks above MA_20 → b_τ = 0 (no fragility)
  - [ ] Unit test: all stocks perfectly correlated → c_τ = 1
  - [ ] Unit test: `get_adjusted_window` unavailable → `MicroLayerUnavailableError` raised
...
- [ ] **Task 4: Rule-week downweighting, micro score, and IIR circuit breaker**
  ...
  **Acceptance criteria:**
  - [ ] `z_wrob_156()` applies ω_τ = 0.3 for rule/rebalance weeks; ω_τ = 1.0 otherwise
  - [ ] `iir_envelope_with_breaker()` matches spec §6.3 signature exactly: if h_t < 0.25 for 3 consecutive weeks → x_t^{lead} = 0
  - [ ] `h_t^{lead} = 0.5 + x_t^{lead}` ∈ [0.5, 1.0] when no fragility signal; ∈ [0.5, 1.0+] only while IIR decays
  - [ ] Single-week sub-threshold does NOT clear IIR memory

  **Verification:**
  - [ ] Unit test: 2-week recovery (h_t = 0.1, 0.1) does not clear IIR → h_t^{lead} remains elevated
  - [ ] Unit test: 3-week recovery (h_t = 0.1, 0.1, 0.1) clears IIR → h_t^{lead} resets to 0.5
  - [ ] Unit test: δ=0.9 decay correct over 10 weeks with no new signal
...
- [ ] `pytest tests/test_micro_layer.py` passes
- [ ] `pytest tests/test_micro_point_in_time.py` passes (PIT semantics)
- [ ] No imports of `micro_layer` fail

---

### Phase 3: Production Risk Score

- [ ] **Task 5: Full rho_t in `risk_layer.py`**
  ...
  **Acceptance criteria:**
  - [ ] `compute_risk_score(omega_t, s_t, h_t_lead, lambda_rho=0.75)` returns rho_t ∈ [0, 1]
  - [ ] rho_t monotonically increasing in s_t and h_t_lead
  - [ ] rho_t = 0 when s_t = 0 and h_t_lead = 0.5 (no micro signal)
  - [ ] `PRODUCTION_RISK_ENABLED = True`
  - [ ] `compute_ewcorr_78w(s_series, h_lead_series)` returns η_t series (for interpretability only)

  **Verification:**
  - [ ] Unit test: rho_t = 1 - (1-1.0)(1-0.75*1.0) = 1.0 at maximum inputs
  - [ ] Unit test: rho_t ∈ [0,1] over 1000 random (m_t, n_t) pairs
...
- [ ] `pytest tests/test_risk_layer.py` passes
- [ ] `PRODUCTION_RISK_ENABLED is True`

---

### Phase 4: Interpretability Module

- [ ] **Task 6: `interpretability.py`**
  ...
  **Acceptance criteria:**
  - [ ] `InterpretabilityRecord` has fields: `A_t` (attribution), `C_t` (contamination flags), `D_t` (drift diagnostics), `H_t` (module health)
  - [ ] `A_t.H_components = (0.40*L_t, 0.35*T_t, 0.25*P_t)` etc. per §11.1
  - [ ] Contamination flags: c_t^{rule}, c_t^{const}, c_t^{data}, c_t^{micro}, c_t^{drift} per §11.2
  - [ ] Module health: h_t^{macro}, h_t^{exo}, h_t^{micro}, h_t^{state} per §11.4
  - [ ] Pure computation — no I/O in this module

  **Verification:**
  - [ ] Unit test: `build_interpretability()` round-trips all fields without loss
  - [ ] Unit test: `c_t^{drift} = 1` when `|delta_abs_raw| >= 1.8`
...
- [ ] `pytest tests/test_interpretability.py` passes
...
- [ ] **Task 7: Add two missing spec-named tests**

  **Description:** The spec (§4.4, §5.7) names specific test functions that are still absent or named differently.

  **Acceptance criteria:**
  - [ ] `tests/test_covariance.py` contains `test_condition_threshold_reachable`: drives `condition_number_reg` above `COND_WARN_RATIO / eps_rel` (= 9000) with a near-degenerate input
  - [ ] `tests/test_dual_memory.py` contains `test_nan_does_not_propagate_to_covariance`: feeds a NaN-output z_ew series into `RobustEWCov2D.update()` and asserts state is unchanged

  **Verification:**
  - [ ] `pytest tests/test_covariance.py::test_condition_threshold_reachable` passes
  - [ ] `pytest tests/test_dual_memory.py::test_nan_does_not_propagate_to_covariance` passes
...
- [ ] `pytest` (full suite) passes with no failures
- [ ] All spec-named tests from §4.4 and §5.7 present and passing
